...
function setup() {
  ...
  aboutUs=createSprite(600,450,50,50);
  aboutUs.addImage("us",aboutUsImage);
  aboutUs.scale=0.3;

  help=createSprite(985,450,50,50);
...
function draw() {
  ...
  //  clickingSound.play();
    ...
    if(bird.isTouching(balloonGroup)){
      balloonGroup.destroyEach();
      life=life-1;
    }
    if(bird.isTouching(planeGroup)){
      planeGroup.destroyEach();
      life=life-1;
    }
    if(bird.isTouching(shipGroup)){
